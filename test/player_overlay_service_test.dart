...
class _FakePlayerOverlayPlatformChannel
  ...
  final Future<Object?> Function(String method, Map<String, Object?>? arguments)
  _handler;
...
void main() {
  ...
  test('字幕条目会过滤无效时间并裁剪异常长文本', () async {
    ...
    final _FakePlayerOverlayPlatformChannel
    channel = _FakePlayerOverlayPlatformChannel((
      String method,
      Map<String, Object?>? arguments,
    ) async {
      ...
      return <String, Object?>{
        ...
        'message': '',
        'cues': <Object?>[
          <String, Object?>{'fromMs': 1200, 'toMs': 2400, 'content': longText},
          <String, Object?>{'fromMs': 3000, 'toMs': 3000, 'content': '无效区间'},
          <String, Object?>{'fromMs': -100, 'toMs': 1000, 'content': '负数起点'},
        ],
      };
    });
    final NativePlayerOverlayService service = NativePlayerOverlayService(
    ...
    expect(
      result.cues.first.content.runes.length,
      SubtitleCue.maxContentLength,
    );
  ...
  test('弹幕按六分钟段号请求并过滤损坏或超长条目', () async {
    ...
    expect(
      result.entries.first.content.runes.length,
  ...
  test('弹幕段号会按六分钟时间轴分页并限制在安全范围', () {
    expect(DanmakuSegmentLoadResult.segmentIndexForPosition(Duration.zero), 1);
    expect(
  ...
  test('无效段号不会触发原生通道且平台异常不会泄露内部内容', () async {
    ...
    final DanmakuSegmentLoadResult invalidResult = await service
        .loadDanmakuSegment(
          bvid: 'BV1GJ411x7h7',
          cid: 137649199,
          segmentIndex: 0,
        );
    final DanmakuSegmentLoadResult failureResult = await service
        .loadDanmakuSegment(
          bvid: 'BV1GJ411x7h7',
          cid: 137649199,
          segmentIndex: 5,

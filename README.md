...
- Android Media3 直接播放 DASH 视频与音频。
...
当前 APK 用于学习测试。仓库中的 Release 构建仍使用本机调试签名。
...
- 不把 Cookie、播放记录或搜索记录上传到开发者服务器。（根本没有开发者服务器）
- 播放进度、最后分P和搜索记录保存在本机。
...
- [JKVideo](https://github.com/tiajinsha/JKVideo)：优秀的 React Native 第三方 B 站客户端。FocuBili 在研究原生播放器时参考了它的公开实现思路。

...
void WebviewWindow::NotifyWindowClosed() {
  if (destroyed_) {
    return;
  }
  destroyed_ = true;
...
void WebviewWindow::Close() {
  // Programmatic close removes this object from the plugin map. Notify Dart
  // first and disable the user-close erase callback to avoid erasing the same
  // map entry again while its destructor is running.
  NotifyWindowClosed();
...
WebviewWindow::MessageHandler(
  ...
  switch (message) {
    case WM_DESTROY: {
      ...
      // WM_DESTROY may be delivered more than once; Dart onClose completes once.
      if (!destroyed_) {
        NotifyWindowClosed();

...
void WebviewWindowPlugin::HandleMethodCall(
  ...
  if (method_call.method_name() == "create") {
    ...
    result->Success();
  } else if (method_call.method_name() == "setUserAgent") {
    auto *arguments =
        std::get_if<flutter::EncodableMap>(method_call.arguments());
    auto window_id =
        arguments->at(flutter::EncodableValue("viewId")).LongValue();
    auto user_agent = std::get<std::string>(
        arguments->at(flutter::EncodableValue("userAgent")));
    if (!windows_.count(window_id) || !windows_[window_id]->GetWebView()) {
      result->Error("0", "webview window not ready");
      return;
    }
    // Replace the whole UA and surface unsupported WebView2 runtimes to Dart.
    const HRESULT hr = windows_[window_id]->GetWebView()->SetUserAgent(
        utf8_to_wide(user_agent));
    if (FAILED(hr)) {
      result->Error("0", "failed to set user agent");
      return;
    }
    result->Success();
  } else if (method_call.method_name() == "setAllowedNavigationHosts") {
    auto *arguments =
        std::get_if<flutter::EncodableMap>(method_call.arguments());
    auto window_id =
        arguments->at(flutter::EncodableValue("viewId")).LongValue();
    auto hosts = std::get<flutter::EncodableList>(
        arguments->at(flutter::EncodableValue("hosts")));
    if (!windows_.count(window_id) || !windows_[window_id]->GetWebView()) {
      result->Error("0", "webview window not ready");
      return;
    }
    std::vector<std::wstring> wide_hosts;
    wide_hosts.reserve(hosts.size());
    for (const auto &host : hosts) {
      wide_hosts.push_back(utf8_to_wide(std::get<std::string>(host)));
    }
    windows_[window_id]->GetWebView()->SetAllowedNavigationHosts(wide_hosts);
    result->Success();
  ...
  } else if (method_call.method_name() == "getCookiesForUrl") {
    auto *arguments =
        std::get_if<flutter::EncodableMap>(method_call.arguments());
    auto window_id =
        arguments->at(flutter::EncodableValue("viewId")).LongValue();
    auto url = std::get<std::string>(
        arguments->at(flutter::EncodableValue("url")));
    if (!windows_.count(window_id) || !windows_[window_id]->GetWebView()) {
      result->Error("0", "webview window not ready");
      return;
    }
    // Let WebView2 select exactly the Cookies applicable to the nav API URL.
    windows_[window_id]->GetWebView()->GetCookiesForUrl(
        utf8_to_wide(url), std::move(result));
    ...
    // Move ownership out before destruction. Close() completes Dart onClose
    // and disables the WM_DESTROY callback that would otherwise erase twice.
    auto closing_window = std::move(windows_.at(window_id));
    windows_.erase(window_id);

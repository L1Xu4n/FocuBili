...
class WebviewImpl extends Webview {
  ...
  OnNavigationCompletedCallback? _onNavigationCompletedCallback;
  ...
  /// 结束导航状态并通知业务层页面已完成一次顶层导航。
  void onNavigationCompleted(String url, bool isSuccess) {
    _isNavigating.value = false;
    _onNavigationCompletedCallback?.call(url, isSuccess);
  }
  ...
  Future<void> setUserAgent(String userAgent) async {
    ...
    if (normalizedUserAgent.isEmpty) {
      throw ArgumentError.value(userAgent, 'userAgent', 'UA 不能为空。');
    }
    await channel.invokeMethod("setUserAgent", {
      "viewId": viewId,
  ...
  /// 保存顶层导航完成回调，让登录页能够像 Android 的 onPageFinished 一样立即检查会话。
  @override
  void setOnNavigationCompletedCallback(
    OnNavigationCompletedCallback? callback,
  ) {
    _onNavigationCompletedCallback = callback;
  }

  /// Pass trusted hosts to the Windows plugin for synchronous navigation checks.
  @override
  Future<void> setAllowedNavigationHosts(List<String> hosts) {
    return channel.invokeMethod("setAllowedNavigationHosts", {
      "viewId": viewId,
      "hosts": hosts,
    });
  }
  ...
  Future<List<WebviewCookie>> getAllCookies() async {
    return _readCookies('getAllCookies');
  }
  ...
  Future<List<WebviewCookie>> getCookiesForUrl(String url) async {
    ...
    if (uri == null || uri.scheme != 'https' || uri.host.isEmpty) {
      throw ArgumentError.value(url, 'url', 'Cookie 查询地址必须是有效的 HTTPS 地址。');
    }
    return _readCookies('getCookiesForUrl', url: uri.toString());
  }

  /// 调用原生 Cookie 桥接并把返回的键值表统一转换为 Dart Cookie 对象。
  Future<List<WebviewCookie>> _readCookies(
    String method, {
    String? url,
  }) async {
    final result = await channel.invokeListMethod<Map>(method, {
      "viewId": viewId,
      if (url != null) "url": url,
    });

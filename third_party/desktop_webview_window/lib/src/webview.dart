...
/// Callback invoked after a top-level navigation, including its URL and success flag.
typedef OnNavigationCompletedCallback = void Function(
  String url,
  bool isSuccess,
);
...
abstract class Webview {
  ...
  /// Install JavaScript before the next document starts and wait until native registration finishes.
  ...
  /// Replace the complete user-agent before the first navigation.
  ...
  /// Register a callback for completed top-level navigations.
  void setOnNavigationCompletedCallback(
    OnNavigationCompletedCallback? callback,
  );

  /// Configure exact HTTPS hosts that Windows may navigate to without replaying requests.
  ...
  /// Read only the Cookies that WebView2 would send to the specified URL.
  Future<List<WebviewCookie>> getCookiesForUrl(String url);
}
